...
@interface CircleCounterView : UIView {
    ...
    float startSeconds;         // Starting seconds
    ...
    id<CircleCounterViewDelegate> __unsafe_unretained mDelegate;
    
    BOOL mCircleIncre;          //< Default NO, the circle is drawed incrementally, otherwise decrementally
}

@property (unsafe_unretained) id<CircleCounterViewDelegate> delegate;
...
@property (nonatomic, assign) float timerStart;
